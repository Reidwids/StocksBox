...
exports.getVisitProfile = async (req, res) => {
  ...
  if (user.userGainsHistTimestamps.length < 10) {
    ...
    for (
      let i = user.userGainsHist.length - 10;
      i < user.userGainsHist.length;
      i++
    ) {
      let tempArr = [];
      tempArr.push(moment(user.userGainsHistTimestamps[i]).format("D/M/YY/LT"));
  ...
  res.render("profile/visitProfile", {
    user,
...
exports.postCreatePortfolio = async (req, res) => {
  ...
  for (let i = 0; i < req.body.typeOfAsset.length; i++) {
    ...
    asset.assetGain =
      data * asset.qtyOwned - asset.priceObtained * asset.qtyOwned;
  ...
  if (!req.user.userGainsHist[0]) {
    ...
    req.user.userGainsHist.push(
      portfolio.gainsHist[0] +
        req.user.userGainsHist[req.user.userGainsHist.length - 1]
    );
  }
...
exports.putUpdatePortfolio = async (req, res) => {
  ...
  if (!Array.isArray(req.body.assetId)) {
    ...
    await Asset.findByIdAndUpdate(req.body.assetId, {
      ...
      assetGain:
        data * req.body.qtyOwned - req.body.priceObtained * req.body.qtyOwned,
    });
    currentGain +=
      data * req.body.qtyOwned - req.body.priceObtained * req.body.qtyOwned;
  } else {
    for (i = 0; i < req.body.assetId.length; i++) {
      ...
      await Asset.findByIdAndUpdate(req.body.assetId[i], {
        ...
        assetGain:
          data * req.body.qtyOwned[i] -
          req.body.priceObtained[i] * req.body.qtyOwned[i],
      });
      currentGain +=
        data * req.body.qtyOwned[i] -
        req.body.priceObtained[i] * req.body.qtyOwned[i];
  ...
  await Portfolio.findByIdAndUpdate(req.body.id, {
    $push: { gainsHist: Math.round(currentGain * 100) / 100 },
  });
  await Portfolio.findByIdAndUpdate(req.body.id, {
    $push: { gainsHistTimestamps: new Date().toISOString() },
  });
...
exports.getDeletePortfolioRemoveGains = async (req, res) => {
  ...
  await updateUserGains(user);
  let portfolio = await Portfolio.findById(req.query.id).populate("assets");
  user.userGainsHist.push(
    user.userGainsHist[user.userGainsHist.length - 1] -
      portfolio.gainsHist[portfolio.gainsHist.length - 1]
  );
...
exports.getLeaderboard = async (req, res) => {
  ...
  for (i = 0; i < allUsers.length; i++) {
    ...
    if (user.portfolios[0]) {
      tempArr.push(
        user.portfolios.sort(function compareFn(a, b) {
          b.gainsHist[b.gainsHist.length - 1] -
            a.gainsHist[a.gainsHist.length - 1];
        })[0].portfolioName
      );
...
async function updateUserGains(user) {
  ...
  for (let portfolio of user.portfolios) {
    ...
    for (let asset of portfolio.assets) {
      ...
      await Asset.findByIdAndUpdate(asset.id, {
        assetGain:
          Math.round(
            (data * asset.qtyOwned - asset.priceObtained * asset.qtyOwned) * 100
          ) / 100,
      });
      currentGain += asset.assetGain;
    }
    gainDiff +=
      currentGain - portfolio.gainsHist[portfolio.gainsHist.length - 1];
  ...
  if (user.userGainsHist[0]) {
    let totalUserDiff =
      user.userGainsHist[user.userGainsHist.length - 1] + gainDiff;
    await User.findByIdAndUpdate(user.id, {
      $push: { userGainsHist: Math.round((totalUserDiff * 100) / 100) },
    });
    await User.findByIdAndUpdate(user.id, {
      $push: { userGainsHistTimestamps: new Date().toISOString() },
    });
  }
}
